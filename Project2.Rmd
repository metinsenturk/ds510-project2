...
2. GLM wiht MaxHR, Oldpeak and Chol variables.
...
Further analysis on variables indicate the following two models are candidate to predict the presense of an heart disease. We implement likelihood ratio test to observe the importance of RestBP and Chol.
...
According to the tests, Chol values are statistically more significant than RestBP values, therefore we picked MaxHR, Oldpeak and Chol to construct our model in classification of heart diseases.
...
According to our model, the odds of having a heart disease is 2.40 times higher in one unit increase of Oldpeak. Likewise, 1 unit of Chol increase in a patient can increase the odds of having a heart disease 1.49 times. 
...
According to the graph above, we determined the following cutoff value for our model.
...
Additionally, we provide the rate of guessing all true and false for comparison with accuracy. 
...
Lastly, in order to eliminate the chance of picking the best subset in the dataset for our selected model, we did a 10 fold cross validation test for our analysis. For that purpose, we created train and test datasets in below. We will train our model with 80% of the dataset, which is roughly the same amount of observations with comparison to our initial analysis.
...
We constructed a 10-fold cross validation with 10 repeats. According to the results of k-fold cross validation test, we found the model we picked presents similiar to our initial analysis. Therefore, we proved that the presented model is behaving as our first analysis.
...
The confision matrix for k-fold cross validation is used to examine accuracy and its significance value.
...
##### Model Validation 
To validate our model, we used test data to present model's success. It is observed that our model acts slightly less accurate with test data than in training.
...
k-fold cross validation results with the test dataset is also presents similar results.
...
#### Support Vector Machine (SVM) Approach
##### Model Analysis
Another approach we used to to predict the presense of the heart disease is support vector machines. SVM divides the dataset into classes with the use of hyperplanes. Below, we found the best SVM with the tune function.
...
According to our analysis, we found the best cost and gamma values as 1 and 2, respectively. 
...
With SVM, we are having better results than logistic regression. 
...
ROC curve for the SVM Model is used to examine an optimal cutoff point. The distribution of models' prediction probabilities for classes are presented in the left graph.
...
```{r svm roc curve on test data, echo=FALSE, include=FALSE, fig.height=3, fig.width=9, fig.align="center", fig.cap="SVM ROC for Test Data"}
...
We compared our results of train and test data and draw the ROC curve. It is observed that the train data presents better results than test data with the use of SVM as a classifier.
