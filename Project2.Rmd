...
## Technical Analysis
There are two different approached used for making the predictions: 
...
### Data Analysis
For proper analysis of the data, the data needs to be passed to various step so as to get a clean data. More the clean more accurate is tha analysis.
```{r data structure}
# examining variables
str(data_raw)
```
...
# best 
lgm_model_1 = glm(AHD ~ MaxHR + Oldpeak + RestBP, data = data_train, family = binomial)
lgm_model_2 = glm(AHD ~ MaxHR + Oldpeak + Chol,   data = data_train, family = binomial)
